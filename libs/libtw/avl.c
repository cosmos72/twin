  ...
  while (P) {
    ...
    if (HL > HR + 1) {
      ...
      if (AVLHeightOf(LL) >= HLR) {
        ...
        /*            X   Y                                       */
        /*                                                        */
        if ((P->AVLLeft = LR->AVLRight))
          LR->AVLRight->AVLParent = P;
        LR->AVLRight = P;
        AVL_Insert(LR, (PP = P->AVLParent), P, root);
        P->AVLParent = LR;
        if ((L->AVLRight = LR->AVLLeft))
          LR->AVLLeft->AVLParent = L;
        LR->AVLLeft = L;
        L->AVLParent = LR;
      ...
      if (AVLHeightOf(RR) >= HRL) {
        ...
        if ((P->AVLRight = RL->AVLLeft))
          RL->AVLLeft->AVLParent = P;
        RL->AVLLeft = P;
        AVL_Insert(RL, (PP = P->AVLParent), P, root);
        P->AVLParent = RL;
        if ((R->AVLLeft = RL->AVLRight))
          RL->AVLRight->AVLParent = R;
        RL->AVLRight = R;
        R->AVLParent = RL;
...
void AVLRemove(tavl P /*node*/, tavl_compare cmp, tavl *root) {
  ...
  if (L && R) {
    ...
    if (!LC) {
      ...
      if ((L->AVLRight = P->AVLRight))
        L->AVLRight->AVLParent = L;
      if ((L->AVLLeft = P->AVLLeft))
        L->AVLLeft->AVLParent = L;
      ...
      if ((R->AVLLeft = P->AVLLeft))
        R->AVLLeft->AVLParent = R;
      if ((R->AVLRight = P->AVLRight))
        R->AVLRight->AVLParent = R;
