...
#define XDRAW_GFX(buf, buflen, col, gfx)                                                           \
  (xmonochrome ? gfx_DrawMono : gfx_DrawColor)(buf, buflen, col, gfx, xbegin, ybegin)
#define XDRAW_GFX1(font, col, gfx)                                                                 \
  (xmonochrome ? gfx_Draw1Mono : gfx_Draw1Color)(font, col, gfx, xbegin, ybegin)
...
static void gfx_DrawMono(myXChar *buf, udat buflen, tcolor col, tcell gfx, int xbegin, int ybegin) {
...
static void gfx_DrawColor(myXChar *buf, udat buflen, tcolor col, tcell gfx, int xbegin,
                          int ybegin) {
  for (; buflen; buf++, buflen--, xbegin += xwfont) {
...
INLINE void X11_Mogrify(dat x, dat y, ldat len) {
  ...
  for (_col = ~TCOLOR(*V); len; x++, V++, oV++, len--) {
    ...
    gfx = HWEXTRA(*V);
    if (buflen &&
        (col != _col || gfx != bufgfx || (ValidOldVideo && *V == *oV) || buflen == TW_SMALLBUFF)) {
...
static byte gfx_LoadPixmap(Pixmap *px, CONST char *name, int nlen, byte strict) {
  ...
  for (i = 0; i < 3 && !ret; i++) {
    if (prefix[i] &&
        (path[i] = (char *)AllocMem(strlen(prefix[i]) + strlen(infix[i]) + nlen + 17))) {
      if (strict)
        sprintf(path[i], "%s%s%.*s_%dx%d.xpm", prefix[i], infix[i], nlen, name, xwfont & 0xFFFF,
                xhfont & 0xFFFF);
      else
        sprintf(path[i], "%s%s%.*s.xpm", prefix[i], infix[i], nlen, name);

      ret = XpmReadFileToPixmap(xdisplay, DefaultRootWindow(xdisplay), path[i], px, NULL,
                                &pxattr) == XpmSuccess;
    }
  }
  ...
  if (!ret) {
    if (!path[0] && !path[1] && !path[2])
      ...
      printk("      gfx_InitHW(): failed to open `" SS "': " SS "\n",
             path[0] ? path[0] : path[1] ? path[1] : path[2], strerror(errno));
  }
...
cleanup:
  if (xmonochrome == ttrue + ttrue)
    xmonochrome =
        opt->file_theme_len >= 4 && !strcmp(opt->file_theme + opt->file_theme_len - 4, "mono");
...
static byte gfx_InitHW(void) {
  ...
  if ((xdisplay = XOpenDisplay(opt.dpy)))
    do {
      ...
      if (gfx_LoadFont(opt.fontname, opt.fontwidth, opt.fontheight) &&
          ...
          (xwindow = XCreateWindow(xdisplay, DefaultRootWindow(xdisplay), 0, 0, xwidth, xheight, 0,
                                   xdepth, InputOutput, DefaultVisual(xdisplay, xscreen),
                                   CWBackPixel | CWEventMask, &xattr)) &&

          (xsgc.foreground = xsgc.background = xcol[0], xsgc.graphics_exposures = False,
           xsgc.font = xsfont->fid,
           xgc = XCreateGC(xdisplay, xwindow,
                           GCFont | GCForeground | GCBackground | GCGraphicsExposures, &xsgc)) &&
          ...
          (xmonochrome ? (xthemesgc.foreground = xthemesgc.background = xcol[0],
                          xthemesgc.graphics_exposures = False, xthemesgc.stipple = xtheme,
                          xthemesgc.fill_style = FillOpaqueStippled,
                          !!(xthemegc = XCreateGC(xdisplay, xwindow,
                                                  GCStipple | GCFillStyle | GCForeground |
                                                      GCBackground | GCGraphicsExposures,
                                                  &xthemesgc)))
                       : ttrue) &&

          (opt.file_root ? (xs_gc.foreground = xs_gc.background = xcol[0],
                            xs_gc.graphics_exposures = False, xs_gc.tile = xroot,
                            xs_gc.fill_style = FillTiled, xs_gc.ts_x_origin = xs_gc.ts_y_origin = 0,
                            !!(xrootgc = XCreateGC(xdisplay, xwindow,
                                                   GCTile | GCFillStyle | GCTileStipXOrigin |
                                                       GCTileStipYOrigin | GCForeground |
                                                       GCBackground | GCGraphicsExposures,
                                                   &xs_gc)))
                         : ttrue) &&

          (opt.file_bg ? (xs_gc.foreground = xs_gc.background = xcol[0],
                          xs_gc.graphics_exposures = False, xs_gc.tile = xbg,
                          xs_gc.fill_style = FillTiled, xs_gc.ts_x_origin = xs_gc.ts_y_origin = 0,
                          !!(xbggc = XCreateGC(xdisplay, xwindow,
                                               GCTile | GCFillStyle | GCTileStipXOrigin |
                                                   GCTileStipYOrigin | GCForeground | GCBackground |
                                                   GCGraphicsExposures,
                                               &xs_gc)))
                       : ttrue) &&
...
static void gfx_QuitHW(void) {
  ...
  if (xthemegc != None)
    XFreeGC(xdisplay, xthemegc);
  if (xroot != None)
    XFreePixmap(xdisplay, xroot);
  if (xbg != None)
    XFreePixmap(xdisplay, xbg);
  if (xwindow != None) {
