    ...
    if (C->W) {
      ...
      if (C->i >= C->Left + HasBorder && C->i <= C->Rgt - HasBorder && C->j >= C->Up + HasBorder &&
        ...
        if (IS_WINDOW(C->W) &&
          ...
          C->Pos =
              Act(FindBorder, (window)C->W)((window)C->W, C->i - C->Left, C->j - C->Up, 0, NULL);
      }
    }
    /*
...
static byte ActivateKeyState(wm_ctx *C, byte State) {
  ...
  case STATE_MENU:
    ...
    case TW_Return:
      ...
      break;
    case TW_Left:
      if (depth > 0 && (All->SetUp->Flags & SETUP_MENU_RELAX) && (W = (window)M->Parent) &&
          IS_WINDOW(W) && (M = W->MenuItem) && (W = (window)M->Parent) && IS_WINDOW(W))
...
EXTERN_C byte InitModule(module Module) {
  ...
  if ((WM_MsgPort = Do(Create, MsgPort)(FnMsgPort, 2, "WM", 0, 0, 0, WManagerH)) &&
    ...
    if (RegisterExt(WM, MsgPort, WM_MsgPort)) {
      ...
      if ((MapQueue = Do(Create, MsgPort)(FnMsgPort, 11, "WM MapQueue", 0, 0, 0,
                                          (void (*)(msgport))NoOp))) {
