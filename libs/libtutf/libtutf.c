...
#define _NLIST(EL)                                                                                 \
  ...
  EL(T_MAP(CP1258))                                                                                \
                                                                                                   \
  EL(T_MAP(ADOBE_STDENC))                                                                          \
  EL(T_MAP(ADOBE_SYMBOL))                                                                          \
                                                                                                   \
  EL(T_MAP(MAC_ARABIC))                                                                            \
  EL(T_MAP(MAC_CELTIC))                                                                            \
  EL(T_MAP(MAC_CENTRALEUROPE))                                                                     \
  EL(T_MAP(MAC_CROATIAN))                                                                          \
  EL(T_MAP(MAC_CYRILLIC))                                                                          \
  EL(T_MAP(MAC_DEVANAGARI))                                                                        \
  EL(T_MAP(MAC_DINGBATS))                                                                          \
  EL(T_MAP(MAC_FARSI))                                                                             \
  EL(T_MAP(MAC_GAELIC))                                                                            \
  EL(T_MAP(MAC_GREEK))                                                                             \
  EL(T_MAP(MAC_GUJARATI))                                                                          \
  EL(T_MAP(MAC_GURMUKHI))                                                                          \
  EL(T_MAP(MAC_HEBREW))                                                                            \
  EL(T_MAP(MAC_ICELANDIC))                                                                         \
  EL(T_MAP(MAC_INUIT))                                                                             \
  EL(T_MAP(MAC_ROMAN))                                                                             \
  EL(T_MAP(MAC_ROMANIAN))                                                                          \
  EL(T_MAP(MAC_SYMBOL))                                                                            \
  EL(T_MAP(MAC_THAI))                                                                              \
  EL(T_MAP(MAC_TURKISH))                                                                           \
                                                                                                   \
  EL(T_MAP(MS_MAC_CYRILLIC))                                                                       \
  EL(T_MAP(MS_MAC_GREEK))                                                                          \
  EL(T_MAP(MS_MAC_ICELANDIC))                                                                      \
  EL(T_MAP(MS_MAC_LATIN2))                                                                         \
  EL(T_MAP(MS_MAC_ROMAN))                                                                          \
  EL(T_MAP(MS_MAC_TURKISH))                                                                        \
                                                                                                   \
  EL(T_MAP(APL_ISO_IR_68))                                                                         \
  EL(T_MAP(ATARIST))                                                                               \
  EL(T_MAP(JISX0201))                                                                              \
  EL(T_MAP(HP_ROMAN8))                                                                             \
  EL(T_MAP(KOI8_R))                                                                                \
...
#define DECL_NAME(ch)                                                                              \
  static const char *T_CAT(names_, ch)[] = {T_CAT(T_NAME_, ch), T_CAT(T_ALIASES_, ch), NULL};
_LIST(DECL_NAME)
#undef DECL_NAME
...
static int strloosecmp(const char *s1, const char *s2) {
  byte c1, c2;
