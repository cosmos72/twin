...
static ttdata protected_Create_ttdata(ttcomponent c, TT_ARG_READ ttbyte *key, ttopaque key_len,
                                      ttany data);
...
ttuint TTLibraryVersion(void) { return TT_PROTOCOL_VERSION; }

TT_DECL_MAGIC(TTMagicData);

ttbyte TTCheckMagic(TT_CONST ttbyte id[]) {
  if (TTCmpMem(id + 1, TTMagicData + 1,
               (id[0] < TTMagicData[0] ? id[0] : TTMagicData[0]) - 2 - sizeof(ttuint))) {
...
static ttuint PrintInitError(tthw HW, TT_CONST ttbyte *target, ttbyte module) {
  ...
  fprintf(stderr, "TT-ERROR: %s display target%s `%s' failed:\n\t%s%s\n",
          module ? "loading" : "compiled in", module ? " module" : "",
          target ? target : (TT_CONST ttbyte *)"(NULL)", TTStrError(L->E),
          TTStrErrorDetail(L->E, L->S));
  return 0;
}
...
static void InitClasses(ttclasses Classes) {
  ...
  for (i = order_first; i < order_last; i++) {
    ...
    for (j = ttclass_sizes[0]; j < ttclass_sizes[i - order_first]; j += sizeof(void *)) {
      if (TTAssertAlways((c1 = *src1) ||
                         ((j < ttclass_sizes[isuper]) && ((c1 = *super2) || (c1 = *super1)))))
        *dst1 = c1;
...
void TTClose(void) { Close(TT_FALSE); }

void TTCloseQuickNDirty(void) { Close(TT_TRUE); }
...
static ttclasses module_InitHW(tthw *HW) {
  ...
  TT_CONST ttbyte *name[] = {
      "twin_tunnel", "twin", "gtk", "X11", NULL, NULL,
  };
...
#define AUTOTARGET(t)                                                                              \
  { TT_STR(t), TT_CAT(TT_CAT(_TT_, t), _InitHW), TT_TRUE }
...
static ttbyte Subsystems_Init(void) {
  ...
  return _TT_field_Init() && _TT_method_Init();
}
...
static ttbyte BeforeOpenTarget(TT_CONST byte *arg) {
  ...
  if (!TTAssertAlways(sizeof(s_ttclass_ttobj) == TT_OFFSETOF(ttclass_ttobject, New)) ||
...
static ttbyte FixedAssignIdArray(ttopaque ClassId, ttopaque o_n, TT_CONST ttobj *o_array) {
  ...
  if (ClassId >= id_ttclass_first && ClassId < id_ttclass_last &&
      !IdTop[i = ClassId & TT_MAGIC_MASK] && o_n &&
      (o_n <= IdSize[i] || FixedIdListGrow(i, o_n)) != TT_BADID) {
...
#define ttavl2ttlistener(avl)                                                                      \
  ((avl) ? (ttlistener)((ttbyte *)(avl)-TT_OFFSETOF(ttlistener, AVL)) : (ttlistener)0)
...
static void CallFunctionPlain(ttlistener_fn function, ttopaque args_n, ttany *args) {
  ...
  case 1:
    ...
    break;
  case 2:
    ((void (*)(ttany, ttany))function)(args[0], args[1]);
    break;
  case 3:
    ((void (*)(ttany, ttany, ttany))function)(args[0], args[1], args[2]);
    break;
  case 4:
    ((void (*)(ttany, ttany, ttany, ttany))function)(args[0], args[1], args[2], args[3]);
  case 5:
    ((void (*)(ttany, ttany, ttany, ttany, ttany))function)(args[0], args[1], args[2], args[3],
                                                            args[4]);
    break;
  case 6:
    ((void (*)(ttany, ttany, ttany, ttany, ttany, ttany))function)(args[0], args[1], args[2],
...
TT_INLINE ttevent FireOneEvent(TT_ARG_DIE ttevent ev) {
  ...
  if (ev && (o = ev->component) && !(ev->evflags & ttevent_evflags_consumed)) {
    ...
    if (o->listeners && (c = FindListener4Event(o->listeners, ev))) {
...
static void FireChangeEvent(ttcomponent o, ttuint which, ttany value, ttany old_value,
                            ttopaque len) {
  tteventbig ev;

  if ((ev = Create4s4_tteventbig(ttevent_evtype_change, which | (order_ttfield << TT_MAGIC_SHIFT),
...
static void AddTo_ttlistener(ttlistener c, ttcomponent o) {
  ...
  if (c && TTAssert(IS(ttlistener, c)) && o && TTAssert(IS(ttcomponent, o)) && !c->component &&
...
TT_INLINE int CompareTF(ttany t1, ttany f1, ttany t2, ttany f2) {
  return t1 < t2 ? -1 : (t1 > t2 ? 1 : (f1 < f2 ? -1 : f1 > f2 ? 1 : 0));
}
...
static void Activate_tttimer(tttimer o, ttbyte active) {
  ...
  if (active && CompareTF(o->delay_t, o->delay_f, TTD.TNow, TTD.FNow) > 0) {
    /* insert */
    if (o->timer_prev || TTD.FirstT == o || o->timer_next || TTD.LastT == o)
      return;
...
static void AddTo_ttdata(ttdata d, ttcomponent o, ttbyte quickndirty) {
  ...
  if (d && TTAssert(IS(ttdata, d)) && o && TTAssert(IS(ttcomponent, o)) && !d->component &&
...
static ttdata protected_Create_ttdata(ttcomponent c, TT_ARG_READ ttbyte *key, ttopaque key_len,
  ...
  if (c && TTAssert(c && IS(ttcomponent, c)) && key && (dkey = TTCloneStrL(key, key_len + 1))) {
...
static void ComputeDelay(struct timeval *t) {
  if (TTAssert(TTD.FirstT)) {
    ...
    printf("compute timer 0x%X\t{%ld, %ld}\n", (unsigned)TTD.FirstT->id, (long)t->tv_sec,
           (long)t->tv_usec);
#endif
  }
}
...
#ifdef DEBUG_TT_TIMER
      printf("select(%d, [%d], NULL, NULL, {%ld, %ld}", fd + 1, fd, (long)t.tv_sec,
             (long)t.tv_usec);
      fflush(stdout);
#endif
      UNLK;
...
ttbyte TTMainLoop(void) { return MainLoop(TT_TRUE); }
