...
void Tw_Create4MenuRow(tw_d TwD, twindow a1, udat a2, byte a3, uldat a4, TW_CONST byte *a5) {
	...
	if (InitRS(TwD)) {
            My = (0 + sizeof(uldat) + sizeof(udat) + sizeof(byte) + sizeof(uldat) + (len5 = (a4) * sizeof(byte)) );
            if (WQLeft(My)) {
                Push(s,uldat,a1); Push(s,udat,a2); Push(s,byte,a3); Push(s,uldat,a4); PushV(s,len5,a5); 
...
void Tw_FocusSubWidget(tw_d TwD, twidget a1) {
    ...
    uldat My;
    LOCK;
    if (Fd != TW_NOFD && ((My = id_Tw[order_FocusSubWidget]) != TW_NOID ||
		       (My = FindFunctionId(TwD, order_FocusSubWidget)) != TW_NOID)) {
	if (InitRS(TwD)) {
            My = (0 + sizeof(uldat) );
            if (WQLeft(My)) {
                Push(s,uldat,a1); 
	            Send(TwD, (My = NextSerial(TwD)), id_Tw[order_FocusSubWidget]);
                    UNLK;return;
            }
	}
	/* still here? must be out of memory! */
	...
	FailedCall(TwD, TW_ENO_FUNCTION, order_FocusSubWidget);
...
byte Tw_SetServerUid(tw_d TwD, uldat a1, byte a2) {
    ...
    uldat My;
    LOCK;
    if (Fd != TW_NOFD && ((My = id_Tw[order_SetServerUid]) != TW_NOID ||
		       (My = FindFunctionId(TwD, order_SetServerUid)) != TW_NOID)) {
	if (InitRS(TwD)) {
            My = (0 + sizeof(uldat) + sizeof(byte) );
            if (WQLeft(My)) {
                Push(s,uldat,a1); Push(s,byte,a2); 
	            Send(TwD, (My = NextSerial(TwD)), id_Tw[order_SetServerUid]);
                    if ((MyReply = (void *)Wait4Reply(TwD, My)) && (INIT_MyReply MyCode == OK_MAGIC)) {
			if (MyLen == 2*sizeof(uldat) + sizeof(byte ))
#if TW_CAN_UNALIGNED != 0
			    a0 = *(byte  *)MyData;
			    ...
			    FailedCall(TwD, TW_ESTRANGE_CALL, order_SetServerUid);
			    a0 = (byte )TW_NOID;
			}
		    } else {
			FailedCall(TwD, MyReply && MyCode != (uldat)-1 ?
				   TW_EFAILED_ARG_CALL : TW_EFAILED_CALL, order_SetServerUid);
			a0 = (byte )TW_NOID;
		    }
		    if (MyReply)
			KillReply(TwD, (byte *)MyReply, MyLen);
	...
	FailedCall(TwD, TW_ENO_FUNCTION, order_SetServerUid);
