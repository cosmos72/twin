...
TT_INLINE void X11_Mogrify(Window w, ttshort x, ttshort y, ttattr *Text, ttattr *OldText,
                           ttuint len, ttbyte ValidOldText) {
  ttattr *V, *oV;
  ttcol col;
  ttushort buflen = 0;
  ttfont f;
...
static ttwidget X11_Build_ttwidget(ttwidget o) {
  if ((o->native = (opaque)XCreateSimpleWindow(dpy, root, o->x * xwfont, o->y * xhfont,
...
static ttframe X11_Build_ttframe(ttframe o) {
  ...
  if (TClass_default(ttframe)->Build(o)) {
    ...
    XChangeProperty(dpy, (Window)o->native, xWM_PROTOCOLS, XA_ATOM, 32, PropModeReplace,
  ...
  do
    if ((dpy = XOpenDisplay(NULL))) {
      ...
      if (((fontname && (xsfont = XLoadQueryFont(dpy, fontname))) ||
           (xsfont = XLoadQueryFont(dpy, "vga")) || (xsfont = XLoadQueryFont(dpy, "fixed")))) {
        ...
        if ((xgc = XCreateGC(dpy, root, GCFont | GCForeground | GCBackground | GCGraphicsExposures,
                             &xsgc))) {
