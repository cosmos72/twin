...
byte RemoteFlush(uldat Slot) {
  ...
  while (LS.WQlen &&
         ((chunk = write(LS.Fd, LS.WQueue + offset, LS.WQlen)), chunk && chunk != (uldat)-1)) {
    offset += chunk;
    LS.WQlen -= chunk;
  }
...
void RemoteParanoia(void) {
  ...
  if (last_errno != EBADF && last_errno != EINVAL) {
    ...
    if (test < 10) {
      /* solved ? */
      log(WARNING) << "                    ... problem disappeared after a few tries, was:\n"
                      "                        errno = "
                   << last_errno << " (" << Chars::from_c(strerror(last_errno)) << ")\n";
      return;
    }
  }
