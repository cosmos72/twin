...
# define EL(x) +1
    enum {
        n = T_NLIST(CP865,EL) + 1,    /* +1 to manually map T_UTF_16_CHECK_MARK -> T_CP865_SQUARE_ROOT below */
...
# undef EL
    ...
    if (!table) {
	table = utf16_hash_create(Tutf_CP865_to_UTF_16, n, n_power_of_2);

        /* manually map T_UTF_16_CHECK_MARK -> T_CP865_SQUARE_ROOT */
        utf16_hash_insert_at(table, n - 1, T_UTF_16_CHECK_MARK, T_CP865_SQUARE_ROOT);
    }
