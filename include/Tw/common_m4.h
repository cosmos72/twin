...
void Tw_Draw2Widget(tdisplay TwD, twidget W, dat XWidth, dat YWidth, dat Left, dat Up, dat Pitch,
                    TW_CONST char *Text, TW_CONST trune *Font, TW_CONST tcell *Attr);

void Tw_DrawTextWidget(tdisplay TwD, twidget W, dat XWidth, dat YWidth, dat Left, dat Up, dat Pitch,
                       TW_CONST char *Text);
void Tw_DrawTRuneWidget(tdisplay TwD, twidget W, dat XWidth, dat YWidth, dat Left, dat Up,
                        dat Pitch, TW_CONST trune *Font);
void Tw_DrawTCellWidget(tdisplay TwD, twidget W, dat XWidth, dat YWidth, dat Left, dat Up,
...
void Tw_WriteTextGadget(tdisplay TwD, tgadget Gadget, dat XWidth, dat YWidth, TW_CONST char *Text,
                        dat Left, dat Up);
void Tw_SetTextGadget(tdisplay TwD, tgadget Gadget, dat XWidth, dat YWidth, TW_CONST char *Text,
                      dat Left, dat Up);
void Tw_SetTextsGadget(tdisplay TwD, tgadget Gadget, byte bitmap, dat XWidth, dat YWidth,
                       TW_CONST char *Text, dat Left, dat Up);
...
void Tw_WriteTRuneGadget(tdisplay TwD, tgadget Gadget, dat XWidth, dat YWidth,
                         TW_CONST trune *TRune, dat Left, dat Up);
void Tw_SetTRuneGadget(tdisplay TwD, tgadget Gadget, dat XWidth, dat YWidth, TW_CONST trune *TRune,
                       dat Left, dat Up);
void Tw_SetTRunesGadget(tdisplay TwD, tgadget Gadget, byte bitmap, dat XWidth, dat YWidth,
                        TW_CONST trune *TRune, dat Left, dat Up);

tmenuitem Tw_Create4MenuRow(tdisplay TwD, twindow Window, udat Code, byte Flags, ldat Len,
                            TW_CONST char *Text);
 ...
 *
 * returns tfalse only after libtw has paniced,* returns ttrue+ttrue if not all data could be
 * written.
 */
byte Tw_TimidFlush(tdisplay TwD);

/**
 * This is the function you must call to check if there are pending Msgs,* i.e. already received
 * from the socket. Since Msgs can be received even during libtw calls,you cannot rely only on
 * select(Tw_ConnectionFd()) to detect incoming messages,as they could be already queued in the
 * library. So you must call either Tw_PendingMsg() or Tw_PeekMsg() below to check for messages.
 * Returns the first pending Msg,without removing it from the queue,* ot (tmsg)0 if no Msgs are
 * pending.
 */
tmsg Tw_PendingMsg(tdisplay TwD);

/**
 * This is the function you must call to check if there are Msgs available,* i.e. either already
 * received or that can be received non-blocking. Returns the first available Msg,without removing
 * it from the queue,* ot (tmsg)0 if no Msgs are available.
 */
...
tlistener Tw_AddKeyboardListener(tdisplay TwD, twidget W, udat Key, udat ShiftFlags,
                                 tfn_listener Listener, void *Arg);
tlistener Tw_AddMouseListener(tdisplay TwD, twidget W, udat Code, udat ShiftFlags,
                              tfn_listener Listener, void *Arg);

tlistener Tw_AddControlListener(tdisplay TwD, twidget W, udat Code, tfn_listener Listener,
                                void *Arg);
tlistener Tw_AddClientMsgListener(tdisplay TwD, twidget W, udat Code, tfn_listener Listener,
                                  void *Arg);
tlistener Tw_AddDisplayListener(tdisplay TwD, udat Code, tfn_listener Listener, void *Arg);
tlistener Tw_AddWidgetListener(tdisplay TwD, twidget W, udat Code, tfn_listener Listener,
                               void *Arg);
tlistener Tw_AddGadgetListener(tdisplay TwD, twidget W, udat Code, tfn_listener Listener,
                               void *Arg);

tlistener Tw_AddMenuListener(tdisplay TwD, twidget W, tmenu Menu, udat Code, tfn_listener Listener,
                             void *Arg);
...
tlistener Tw_CreateListener(tdisplay TwD, udat Type, tevent_any E, tfn_listener Listener,
                            void *Arg);
