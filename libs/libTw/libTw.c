...
static tany CallBExtension(tdisplay TwD, textension eid, uldat space, tany args_n, tsfield a) {
  ...
  if (Fd != TW_NOFD && (My = id_Tw[o]) != TW_NOID &&
    ...
    space += sizeof(uldat) + sizeof(topaque) +
             2 * sizeof(byte); /* a[0] (eid) + a[1] (space) + return type */
...
tany Tw_CallVExtension(tdisplay TwD, textension eid, TW_CONST byte *proto, topaque args_n,
                       va_list vargs) {
  struct s_tsfield a[TW_MAX_ARGS_N];
  uldat space;
  udat N;
