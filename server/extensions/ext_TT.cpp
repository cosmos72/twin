...
static ttfunction_fn method_unsafe_addresses[] = {
    (ttfunction_fn)TTExitMainLoop, (ttfunction_fn)TTCallA_ttmethod, (ttfunction_fn)TTCallL_ttmethod,
    (ttfunction_fn)TTCallR_ttmethod, (ttfunction_fn)TTCallV_ttmethod};
...
TT_INLINE ttbyte ext_TT_init_data(void) {
  ...
  if ((method_first = (ttopaque)TTGetFirst(TTClass_ttmethod)) &&
    ...
    for (n = 0; n < method_unsafe_n; n++) {
      method_unsafe_array[n] = (ttopaque)TTGetByAddress_ttmethod(method_unsafe_addresses[n]);
    }
...
#define C(t1, t2)                                                                                  \
  if (Class == TT_CAT(TTClass_, t1))                                                               \
  return TT_CAT(TWS_, t2)
...
TT_INLINE ttbyte FillTypes(ttopaque m, ttopaque *args_n, tsfield tws, ttarg *args) {
  ...
  if (m >= method_first && m < method_last && (v = TTGetParameterType_ttmethod((ttmethod)m)) &&
      (!(n = TTGetArrayN_ttvector(v)) || (proto = TTGetArray_ttvector(v))) && n <= *args_n) {
...
static tany ext_TT_CallBExtension(extension Extension, topaque len, TT_const byte *data,
                                  void *return_type) {
  ...
  if (Ext(Socket, DecodeExtension)(&len, &data, &args_n, tws) &&
      ...
      (m = (ttmethod)(ttopaque)tws->TWS_field_scalar) && IsSafeMethod((ttopaque)m) &&
      ...
      (CopyArgsTWS2TT(args_n, tws, args), TTCheckArraySizesA_ttmethod(m, args_n, args)) &&
