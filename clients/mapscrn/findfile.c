...
static struct decompressor {
  ...
  char *cmd;
} decompressors[] = {{".gz", "gzip -d -c"}, {".bz2", "bzip2 -d -c"}, {0, 0}};

static FILE *pipe_open(struct decompressor *dc) {
  char *pipe_cmd;
  FILE *fp;
...
static FILE *maybe_pipe_open(void) {
  FILE *fp;
  char *t;
...
static FILE *findfile_in_dir(char *fnam, char *dir, int recdepth, char **suf) {
  FILE *fp = NULL;
  DIR *d;
  struct dirent *de;
  char *ff, *fdir, *p, *q, **sp;
  ...
  while ((de = readdir(d)) != NULL) {
    ...
    if (recdepth || okdir) {
      ...
      if (stat(a, &statbuf) == 0 && S_ISDIR(statbuf.st_mode)) {
        if (okdir)
