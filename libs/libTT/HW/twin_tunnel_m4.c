...
static ttbyte twin_tunnel_AddY_ttvector(ttvector o, ttopaque pos, ttopaque value_n,
                                        TT_ARG_READ TT_ARG_ARRAY((_P(2))) ttany *values) {
  return (ttbyte)TunnelCall(ttmethod_AddY_ttvector, (ttany)o, (ttany)pos, (ttany)value_n,
                            (ttany)values);
}
static ttopaque twin_tunnel_ContainsValue_ttvector(TT_ARG_READ ttvector o, ttopaque pos_start,
                                                   ttany value) {
  return (ttopaque)TunnelCall(ttmethod_ContainsValue_ttvector, (ttany)o, (ttany)pos_start,
                              (ttany)value);
}
static ttbyte twin_tunnel_RemoveRange_ttvector(ttvector o, ttopaque pos_start, ttopaque pos_n) {
  return (ttbyte)TunnelCall(ttmethod_RemoveRange_ttvector, (ttany)o, (ttany)pos_start,
                            (ttany)pos_n);
}
/* ttfield */
...
static void
twin_tunnel_Draw_ttvisible(ttvisible o, ttshort x, ttshort y, ttshort w, ttshort h, ttshort pitch,
  ...
  TunnelCall(ttmethod_Draw_ttvisible, (ttany)o, (ttany)x, (ttany)y, (ttany)w, (ttany)h,
             (ttany)pitch, (ttany)(opaque)asciidata, (ttany)(opaque)fontdata,
             (ttany)(opaque)attrdata);
}
static void twin_tunnel_BuiltinRepaint_ttvisible(ttvisible o, ttshort x, ttshort y, ttshort w,
                                                 ttshort h) {
...
static void twin_tunnel_SetFontD_ttlabel(ttlabel o, ttopaque text_len,
...
static void twin_tunnel_SetAttrD_ttanybutton(ttanybutton o, ttshort w, ttshort h, ttshort pitch,
                                             TT_ARG_DIE TT_ARG_ARRAY((_P(3), *, _P(4)))
                                                 ttattr *text) {
  TunnelCall(ttmethod_SetAttrD_ttanybutton, (ttany)o, (ttany)w, (ttany)h, (ttany)pitch,
             (ttany)(opaque)text);
}
/* ttbutton */
...
#define C(t1, t2)                                                                                  \
  case TT_CAT(order_, t1):                                                                         \
    tws_type |= TT_CAT(TWS_, t2);                                                                  \
    break;
    C(void, void);
    C(ttbyte, byte);
    C(ttshort, dat);
...
static ttobject OverwriteIdFromNative(ttobject o) {
  ...
  if (o->native && o->native == (ttopaque)o->native &&
      (o->native >> TT_MAGIC_SHIFT) == (o->id >> TT_MAGIC_SHIFT) && !ID2OBJ(o->native)) {
...
static void debug_method(ttopaque args_n, tsfield a) {
  ...
  for (i = 2; i <= args_n + 1; i++) {
    ...
    if (a[i].type == (TWS_vec | TWS_vecW | TWS_byte))
      ...
      printf("0x%08lX[%ld]", (unsigned long)a[i].TWS_field_vecV,
             (unsigned long)a[i].TWS_field_vecL);
...
static ttany TunnelCall(opaque o, ...) {
  ...
  for (a = ta + 2, i = 0; i < args_n; i++) {
    ...
    if (type == id_ttclass_void) {
      ...
      a[i].TWS_field_vecL = aa[i].size = TTGetArraySizeA_ttmethod((tt_obj)M->id, i, args_n, aa);

    } else {
      if ((Class = ID2CLASS(_Class)) && ISCLASS(ttobj, Class) && /* we are sending a libTT obj */
          (O = (ttobject)(opaque)val) &&
          Is(Class, O)) { /* and it is a valid obj of the correct type */

        if (IS(ttobject, O) && !(O->oflags & (ttobject_oflags_static | ttobject_oflags_const)))
...
static s_ttclasses twin_tunnel_TTClasses = {
...

    {
        ...
        /* extends ttclass_ttobj */
        (void *)NULL /* UNDEFINED: New_ttapplication */, twin_tunnel_Build_ttapplication,
